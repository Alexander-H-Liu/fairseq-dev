...
### Size of codebook
codebook_size=512  # 512>1024>2048>256>128
### Targeting top K layers, each with a codebook
top_k=8 # 8>10>6>12>4>2 (not sure if 10 & 12 is going to run OOM)
### Codebook init. method
normal_init_codebook=false # false: random then Instance norm, true: normal(0,1/|codebook|**2)
### Masking prob
mask_prob=0.80      # worth tuning?
### TBD
freeze_codebook_step=200000 # see if training is unstable before 200k
codebook_init_decay=0.9 # so far so good (unstable is caused by LR or teacher EMA, not codebook decay)
...
hydra/launcher=submitit_slurm +run=slurm_2_aws \
task.data=$data_root \
